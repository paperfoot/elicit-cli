...
/// Surface rate-limit headers to STDERR in human mode (so stdout stays clean
/// for piping). In JSON mode this is a no-op — the rate-limit data is included
/// in the success envelope's `data` by the command instead. Suppressed by
/// `--quiet`.
pub fn emit_rate_limit(ctx: Ctx, rl: &crate::api::RateLimit) {
    if rl.is_empty() || ctx.quiet {
        return;
    }
    if let Format::Human = ctx.format {
        use owo_colors::OwoColorize;
        let remaining = rl
            .remaining
            .map(|r| r.to_string())
