...
pub fn run(ctx: Ctx, check: bool, config: &AppConfig) -> Result<(), AppError> {
    ...
    if check {
        // `update --check` must always be safe and complete with exit 0 even if
        // the release feed is unreachable or the repo has no releases yet (the
        // AGENTS.md update standard: "exit 0 when the check completes"). A
        // failure here means "could not determine latest", NOT a hard error —
        // surface it as a completed-but-unknown check. Exit 1 is reserved for
        // the apply path below.
        match updater.get_latest_release() {
            ...
            Err(e) => {
                ...
                output::print_success_or(ctx, &result, |r| {
                    println!(
                        "Could not check for updates (current v{}): {e}",
                        r.current_version
                    );
                    println!("The release feed may be unreachable or have no releases yet.");
                });
            }
        }
    } else {
